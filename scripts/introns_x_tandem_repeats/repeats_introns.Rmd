...
n_intron_is_noneq_mult = c()
n_intron_is_eq = c()
for(t in 1:n_trials){
	# repeat_lengths = sample(hits.unique$R_size)
	repeat_lengths = sample(hits.unique$R_size)
	# n_intron_is_mult = c(n_intron_is_mult, sum(intron_lengths %% repeat_lengths == 0))
	# n_intron_is_noneq_mult = c(n_intron_is_noneq_mult, sum(intron_lengths > repeat_lengths & intron_lengths %% repeat_lengths == 0))
	# n_intron_is_eq = c(n_intron_is_eq, sum(intron_lengths == repeat_lengths))
	n_intron_is_mult = c(n_intron_is_mult, nrow(intron_lengths %>% filter(I_length %% repeat_lengths == 0) %>% distinct(I_coords)))
	n_intron_is_mult_rmult3 = c(n_intron_is_mult, nrow(intron_lengths %>% filter(I_length %% repeat_lengths == 0, R_mult3) %>% distinct(I_coords)))
	n_intron_is_noneq_mult = c(n_intron_is_noneq_mult, nrow(intron_lengths %>% filter(I_length > repeat_lengths & I_length %% repeat_lengths == 0) %>% distinct(I_coords)))
	n_intron_is_eq = c(n_intron_is_eq, nrow(intron_lengths %>% filter(I_length == repeat_lengths) %>% distinct(I_coords)))
}
# rm(n_intron_is_mult, n_intron_is_noneq_mult, n_intron_is_eq)
```
...
```{r}
# n_len_mult = sum(hits.unique$I_length %% hits.unique$R_size == 0)
# n_len_noneq_mult = sum(hits.unique$I_length > hits.unique$R_size & hits.unique$I_length %% hits.unique$R_size == 0)
# n_len_eq = sum(hits.unique$I_length == hits.unique$R_size)
...
paste(n_len_mult, n_len_mult_rmult3, n_len_noneq_mult, n_len_eq)
...
paste(noneq_mult_success, noneq_mult_success / n_trials)

eq_success = sum(n_intron_is_eq >= n_len_eq)
paste(eq_success, eq_success / n_trials)
